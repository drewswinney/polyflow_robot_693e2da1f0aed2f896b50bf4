...
in
{
  ...
  environment.etc."polyflow/metadata.json.age" = lib.mkIf (builtins.pathExists ./metadata.json.age) {
    source = ./metadata.json.age;
    mode = "0400";  # Read-only for root
  };

  environment.etc."polyflow/sops-key.txt" = lib.mkIf (builtins.pathExists ./sops-key.txt) {
    source = ./etc/polyflow/sops-key.txt;
    mode = "0400";  # Read-only for root
  };
